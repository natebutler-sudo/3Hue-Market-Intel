...
function CreatorOverlay({ activeView, onClose, onNotice, assets, setAssets, drafts, setDrafts }: { activeView: View; onClose: () => void; onNotice: (message: string) => void; assets: CreatorAsset[]; setAssets: Dispatch<SetStateAction<CreatorAsset[]>>; drafts: CreatorDraft[]; setDrafts: Dispatch<SetStateAction<CreatorDraft[]>> }) {
  ...
  const folderIdsInView = useMemo(() => {
    const descendants = creatorFolders.filter((folder) => folder.parentId === selectedFolder.id).map((folder) => folder.id);
    return [selectedFolder.id, ...descendants];
  }, [selectedFolder.id]);
  const visibleAssets = useMemo(() => assets.filter((asset) => folderIdsInView.includes(asset.folderId) && (!query || `${asset.name} ${asset.kind} ${asset.description} ${asset.source}`.toLowerCase().includes(query.toLowerCase()))), [assets, folderIdsInView, query]);
  const visibleDrafts = useMemo(() => drafts.filter((draft) => folderIdsInView.includes(draft.folderId) && (!query || `${draft.title} ${draft.outputType} ${draft.hue}`.toLowerCase().includes(query.toLowerCase()))), [drafts, folderIdsInView, query]);
  const visibleBrandRules = useMemo(() => brandKitRecords.filter((record) => folderIdsInView.includes(record.folderId)), [folderIdsInView]);
  const createDraft = () => {
  ...
  return <div className="creator-shell" role="dialog" aria-modal="true" aria-label="3HUE creator workspace">
    ...
    <nav className="creator-tabs" aria-label="Creator workspace sections" role="tablist">{(['Create', 'Library', 'Brand kit', 'Campaigns', 'Templates'] as CreatorTab[]).map((item) => <button key={item} role="tab" aria-selected={tab === item} className={tab === item ? 'active' : ''} onClick={() => { setTab(item); if (item === 'Brand kit') setFolderId('brand-kit'); }}>{item}</button>)}</nav>
    <div className="creator-body">
      <aside className="creator-sidebar"><div className="creator-sidebar-heading"><span className="eyebrow">Private folders</span><FolderOpen size={16} /></div>{topLevelFolders.map((folder) => <div key={folder.id}><button className={`creator-folder ${folderId === folder.id ? 'active' : ''}`} onClick={() => { setFolderId(folder.id); setTab(folder.id === 'brand-kit' ? 'Brand kit' : 'Library'); }}><FolderOpen size={15} />{folder.label}</button>{creatorFolders.filter((child) => child.parentId === folder.id).map((child) => <button key={child.id} className={`creator-subfolder ${folderId === child.id ? 'active' : ''}`} onClick={() => { setFolderId(child.id); setTab(child.id === 'brand-templates' ? 'Templates' : child.id.startsWith('brand-') ? 'Brand kit' : 'Library'); }}>{child.label}</button>)}</div>)}<div className="creator-sidebar-note"><ShieldCheck size={14} /><span>Drafts stay private and are not exposed to Experience or Product catalog.</span></div></aside>
      <main className="creator-main">
        ...
        {tab === 'Brand kit' && <section className="creator-brand-view"><div className="creator-view-heading"><div><p className="eyebrow accent-eyebrow">Brand kit</p><h2>{selectedFolder.id === 'brand-kit' ? 'Identity rules and approved starting points' : selectedFolder.label + ' guidance'}</h2><p>Use source-backed identity, logo, color, typography, voice, and usage guidance before drafting. Campaign ideas and draft outputs stay in their own folders.</p></div><label className="creator-upload-button"><UploadCloud size={15} /> Upload brand asset<input type="file" onChange={handleUpload} /></label></div><div className="brand-kit-grid">{visibleBrandRules.map((record: BrandKitRecord) => <article className="brand-kit-card" key={record.id}><div className="creator-record-top"><span className="eyebrow">{record.label}</span><span className={'creator-status-badge ' + (record.status === 'Approved reference' ? 'approved' : 'review')}>{record.status}</span></div><strong>{record.value}</strong><small>{record.source}</small></article>)}{visibleBrandRules.length === 0 && <div className="creator-empty"><ShieldCheck size={22} /><strong>No brand rules in this folder yet.</strong><span>Add the approved guidance when it is available.</span></div>}</div><div className="brand-kit-assets-heading"><div><p className="eyebrow">Available brand assets</p><p>Files already supplied are shown with their source and approval state. Missing variants remain clearly identified in the identity rules.</p></div><span>{visibleAssets.length} assets</span></div><div className="creator-record-grid brand-kit-assets">{visibleAssets.map((asset) => <button className="creator-record-card" key={asset.id} onClick={() => setSelectedAsset(asset)}><div className="creator-record-top"><span className="creator-record-icon">{asset.kind === 'Logo' ? <FileImage size={16} /> : <FileText size={16} />}</span><span className={'creator-status-badge ' + (asset.status === 'Approved reference' ? 'approved' : 'review')}>{asset.status}</span></div><h3>{asset.name}</h3><p>{asset.kind} · {asset.source}</p><small>{asset.updatedAt}</small></button>)}{visibleAssets.length === 0 && <div className="creator-empty"><FolderOpen size={22} /><strong>No supplied files in this folder.</strong><span>Upload a logo, source file, or approved rule into the private library.</span></div>}</div><div className="brand-kit-callout"><ShieldCheck size={17} /><div><strong>Source-aware creation</strong><p>Claims, customer stories, metrics, and framework language should carry their source and approval status into every draft.</p></div></div>{selectedAsset && <aside className="creator-asset-detail"><div className="creator-detail-heading"><div><p className="eyebrow">{selectedAsset.kind}</p><h3>{selectedAsset.name}</h3></div><button className="icon-button" onClick={() => setSelectedAsset(null)} aria-label="Close asset details"><X size={17} /></button></div><p>{selectedAsset.description}</p><div className="creator-detail-list"><span><strong>Status</strong>{selectedAsset.status}</span><span><strong>Source</strong>{selectedAsset.source}</span><span><strong>Updated</strong>{selectedAsset.updatedAt}</span></div></aside>}</section>}
        ...
        {tab === 'Templates' && <section className="creator-template-view"><div className="creator-view-heading"><div><p className="eyebrow accent-eyebrow">Templates</p><h2>Templates will live here when supplied</h2><p>No reusable templates are loaded yet. Add the approved slide, document, proposal, email-signature, or social-frame files when you are ready.</p></div><label className="creator-upload-button"><UploadCloud size={15} /> Upload template<input type="file" onChange={handleUpload} /></label></div><div className="creator-empty creator-template-empty"><Layers3 size={24} /><strong>Awaiting supplied templates</strong><span>The folder is intentionally empty so draft examples do not get mistaken for approved brand templates.</span></div></section>}
      </main>
    </div>
  </div>;
}
...
const collectionCadenceSeconds = 4 * 60 * 60;
const initialCollectionCountdown = 3 * 60 * 60 + 42 * 60;
...
export default function Home() {
  ...
  const [collectionCountdown, setCollectionCountdown] = useState(initialCollectionCountdown);
  ...
  const nextCollectionLabel = formatCountdown(collectionCountdown);

  useEffect(() => {
    const timer = window.setInterval(() => {
      setCollectionCountdown((current) => current > 0 ? current - 1 : collectionCadenceSeconds - 1);
    }, 1000);
